...
resource "aws_subnet" "public_1" {
  vpc_id                  = aws_vpc.main.id
  cidr_block              = cidrsubnet(var.vpc_cidr, 4, 0)  # First /20
...
resource "aws_subnet" "public_2" {
  vpc_id                  = aws_vpc.main.id
  cidr_block              = cidrsubnet(var.vpc_cidr, 4, 1)  # Second /20
...
resource "aws_subnet" "private_1" {
  vpc_id            = aws_vpc.main.id
  cidr_block        = cidrsubnet(var.vpc_cidr, 4, 2)  # Third /20
  availability_zone = data.aws_availability_zones.available.names[0]
...
resource "aws_subnet" "private_2" {
  vpc_id            = aws_vpc.main.id
  cidr_block        = cidrsubnet(var.vpc_cidr, 4, 3)  # Fourth /20
  availability_zone = data.aws_availability_zones.available.names[1]
...
resource "aws_eip" "nat_1" {
  ...
  tags = {
    Name = "${var.app_name}-${var.environment}-nat-eip-1"
  }
...
resource "aws_eip" "nat_2" {
  ...
  tags = {
    Name = "${var.app_name}-${var.environment}-nat-eip-2"
  }
